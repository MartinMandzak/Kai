...
def get_features(data, feature_start, feature_end, target_start, target_end):
    ...
    # FIX 1: Dynamic 't' calculation using feature_end instead of hardcoded date
    t = features_data.groupby('CustomerID')['date'].min().apply(
        lambda x: (pd.to_datetime(feature_end) - x).days
    ).rename('t')
...
# FIX 2: Corrected non-overlapping splits
X_train, y_train = get_features(data, '2011-01-01', '2011-06-11', '2011-06-12', '2011-09-09')
X_test, y_test = get_features(data, '2011-09-10', '2011-12-09', '2011-12-10', '2011-12-09')  # Adjusted test period
...
def evaluate(actual, predictions):
    ...
    print(f"Individual MAE: {mean_absolute_error(actual, predictions)}")
    
    # Error margin metrics
    mape = np.mean(np.abs(residuals / (actual + 1e-6))) * 100  # +1e-6 to avoid division by zero
    ...
    axes[0].set_title('CLV Distribution Comparison')
    
    # Prediction Interval Plot
    axes[1].scatter(actual, predictions, alpha=0.5, label='Predictions')
    axes[1].plot([actual.min(), actual.max()], [actual.min(), actual.max()], 'k--', label='Perfect Prediction')
    axes[1].fill_between(actual, lower_bound, upper_bound, color='orange', alpha=0.2, label='95% Interval')
    axes[1].set_title('Actual vs Predicted CLV with Error Margins')
    axes[1].legend()
    
    plt.tight_layout()
    plt.show()

# Evaluate with error margins
